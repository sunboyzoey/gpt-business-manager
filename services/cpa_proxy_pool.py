...
职责(仅此一项): 给 GPT PRO 账号分配一个代理 IP, 把 proxy_url 写进生成的 CPA
凭证文件。**不做任何实际网络请求 / 浏览器代理 / 取码**, 纯数据标签。

规则:
- 导入格式: socks5://用户名:密码@IP:端口 (每行一个)。
- 1 个代理最多绑定 max_accounts(默认 3) 个账号。
- 账号绑定关系存在 GptProAccountModel.extra_json["cpa_proxy_id"]。
...
from core.db import engine, CpaProxyModel, GptProAccountModel
...
def _acc_extra(acc: GptProAccountModel) -> Dict[str, Any]:
    try:
...
def _is_active_binding(acc: GptProAccountModel) -> bool:
...
def _usage_map(session: Session) -> Dict[int, int]:
    ...
    usage: Dict[int, int] = {}
    accounts = session.exec(select(GptProAccountModel)).all()
    for acc in accounts:
...
def import_proxies(text: str) -> Dict[str, Any]:
    ...
    with Session(engine) as session:
        existing = {p.proxy_url for p in session.exec(select(CpaProxyModel)).all()}
        batch_seen = set()
        for raw in (text or "").splitlines():
            ...
            if norm in existing or norm in batch_seen:
                ...
                continue
            batch_seen.add(norm)
            session.add(CpaProxyModel(proxy_url=norm))
            added += 1
        session.commit()
...
def list_proxies() -> List[Dict[str, Any]]:
    with Session(engine) as session:
        proxies = session.exec(select(CpaProxyModel)).all()
...
def delete_proxy(proxy_id: int) -> bool:
    with Session(engine) as session:
        p = session.get(CpaProxyModel, proxy_id)
        if not p:
            return False
        session.delete(p)
        session.commit()
...
def get_account_proxy_url(account_id: int) -> str:
    ...
    with Session(engine) as session:
        ...
        if not pid:
            return ""
        p = session.get(CpaProxyModel, int(pid))
...
def assign_proxy(account_id: int) -> str:
    ...
    with Session(engine) as session:
        ...
        usage = _usage_map(session)
        candidates = [
            p for p in session.exec(select(CpaProxyModel)).all()
