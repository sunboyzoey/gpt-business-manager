...
def probe_uploaded_auth_file(
    ...
    *,
    ...
    """Verify one exact, newly uploaded Codex auth-file through ``wham/usage``.

    Upload callers already know the exact remote filename, so this deliberately
    does not use email substring matching.  A short retry is allowed only for
    management transport/eventual-listing failures.  Authoritative upstream
    failures such as ``token_invalidated`` are returned immediately so callers
    can compensate the confirmed remote write instead of activating it locally.
    """
    name = str(auth_name or "").strip()
    chatgpt_account_id = str(account_id or "").strip()
    checked_at = _utcnow_iso()
    if not name:
        return {
            "last_probe_at": checked_at,
            "last_probe_status_code": 0,
            "last_probe_error_code": "",
            "last_probe_message": "缺少精确 auth-file 名称",
            "remote_state": "probe_skipped",
        }
    if not chatgpt_account_id:
        return {
            ...
            "last_probe_status_code": 0,
            "last_probe_error_code": "",
            "last_probe_message": "缺少 Chatgpt-Account-Id",
            "remote_state": "probe_skipped",
        }
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            last_result = {
                "last_probe_at": _utcnow_iso(),
                "last_probe_status_code": 0,
                "last_probe_error_code": "",
                "last_probe_message": str(exc)[:500],
            ...
            if len(exact) > 1:
                return {
                    "last_probe_at": _utcnow_iso(),
                    "last_probe_status_code": 0,
                    "last_probe_error_code": "duplicate_auth_file",
                    "last_probe_message": "CPA 返回多个同名 auth-file",
            ...
            if not exact:
                last_result = {
                    ...
                    "last_probe_status_code": 0,
                    "last_probe_error_code": "",
                    "last_probe_message": "CPA 尚未返回刚上传的 auth-file",
                    "remote_state": "not_found",
                }
            else:
                matched = exact[0]
                if bool(matched.get("disabled")):
                    return {
                        "last_probe_at": _utcnow_iso(),
                        "last_probe_status_code": 0,
                        "last_probe_error_code": "auth_file_disabled",
                        "last_probe_message": "刚上传的 auth-file 处于停用状态",
                        "remote_state": "disabled",
                    }
                auth_index = str(matched.get("auth_index") or "").strip()
                try:
                    result = _probe_remote_auth(
                        auth_index,
                        chatgpt_account_id,
                        api_url=api_url,
                        api_key=api_key,
                    ...
                    # Any upstream response is authoritative.  In particular,
                    # never retry a revoked token and accidentally call a later
                    # credential that replaced this exact file.
                    return result
    ...
    return last_result or {
        ...
        "last_probe_status_code": 0,
        "last_probe_error_code": "",
        "last_probe_message": "CPA 凭证健康检查未完成",
        "remote_state": "unreachable",
    }
