...
CONFIG_KEYS = {
    "enabled": ("gpt_plan_cpa_enabled", "0"),
    "device_id": ("gpt_plan_cpa_device_id", ""),
    "api_url": ("gpt_plan_cpa_api_url", ""),
    "api_key": ("gpt_plan_cpa_api_key", ""),
    "threshold": ("gpt_plan_cpa_threshold", "10"),
    "refund_days": ("gpt_plan_cpa_refund_days", "2"),
    "interval_minutes": ("gpt_plan_cpa_interval_minutes", "10"),
    "max_per_run": ("gpt_plan_cpa_max_per_run", "3"),
    "auto_buy": ("gpt_plan_cpa_auto_buy", "0"),
    "card_max_uses": ("gpt_plan_card_max_uses", "6"),
    "recycle_hour": ("gpt_plan_cpa_recycle_hour", "0"),  # 每天几点跑退款回收(0-23)
    "devices": ("gpt_plan_cpa_devices", "[]"),  # JSON: [{device_id, threshold, api_url?, api_key?, refund_enabled, replenish_enabled, auto_buy, refund_days}]
}
...
def _overdue_pro(devices: list) -> list:
    ...
    """
    import json
    from core.db import engine, GptPlanAccountModel as GptProAccountModel
...
def _count_cards() -> int:
    try:
        from core.db import engine, GptPlanCardModel as CardModel
        from sqlmodel import Session, select
        with Session(engine) as s:
...
def _refund_and_remove(account_id: int) -> tuple[bool, str]:
    ...
    from api.gpt_plan_operations import refund_account, GptProActionRequest
    try:
...
def _referral_cf_domain() -> str:
    ...
    try:
        from api.gpt_plan_operations import _load_cf_domains
...
def _run_referral_reset(account_id: int, cf_domain: str, browser_mode: str = "headless") -> bool:
    ...
    try:
        from api.gpt_plan_operations import _do_invite_auto, GptProReferralInviteAutoRequest
    except Exception as exc:
