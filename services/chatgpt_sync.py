...
def upload_chatgpt_account_to_cpa(account: Any, api_url: str | None = None, api_key: str | None = None, priority: int = 0) -> tuple[bool, str]:
    try:
        extra = _get_account_extra(account)
        oauth_file = str(extra.get("oauth_file") or "").strip()
        email = str(getattr(account, "email", "") or extra.get("email") or "?")
        if oauth_file:
            if os.path.isfile(oauth_file):
                from platforms.chatgpt.cpa_upload import upload_auth_file_to_cpa

                filename = os.path.basename(oauth_file) or f"{email}.json"
                with open(oauth_file, "rb") as fh:
                    file_content = fh.read()
                # 文件里必须含 refresh_token，AT-only 文件 10 天后失效，拒绝上传
                try:
                    parsed = json.loads(file_content.decode("utf-8"))
                except Exception:
                    parsed = {}
                if not (isinstance(parsed, dict) and str(parsed.get("refresh_token") or "").strip()):
                    return False, "OAuth 文件缺少 refresh_token，请先点「补 RT」拿到 RT 后再上传"
                if priority:
                    try:
                        token_data = parsed if isinstance(parsed, dict) else json.loads(file_content.decode("utf-8"))
                        if isinstance(token_data, dict):
                            token_data["priority"] = int(priority)
                        ...
                        logger.warning("[CPA] %s: OAuth 文件 priority 写入失败，按原文件上传: %s", email, exc)
                logger.info("[CPA] %s: 直接上传 OAuth 文件 %s", email, oauth_file)
                return upload_auth_file_to_cpa(
                    filename=filename,
                    file_content=file_content,
                    api_url=api_url,
                    api_key=api_key,
                )
            logger.warning("[CPA] %s: OAuth 文件不存在，回退到 token JSON 生成: %s", email, oauth_file)

        sync_account = build_chatgpt_sync_account(account)
        email = getattr(sync_account, "email", "?")
        if not getattr(sync_account, "access_token", ""):
            return False, "账号缺少 access_token"
        if not getattr(sync_account, "refresh_token", ""):
            return False, "账号缺少 refresh_token，请先点「补 RT」拿到 RT 后再上传"
        if priority:
            sync_account.cpa_priority = priority

        from platforms.chatgpt.cpa_upload import generate_token_json, upload_to_cpa

        token_data = generate_token_json(sync_account)
        logger.info(f"[CPA] {email}: 上传中... account_id={token_data.get('account_id', '?')}")
        upload_filename = f"{email}.json" if email and email != "?" else None
...
def backfill_chatgpt_account_to_cpa(
    ...
    *,
    ...
    from services.external_sync import should_sync_to_cpa as _should_sync

    # BUSINESS 过滤
    _ok, _reason = _should_sync(account)
    if not _ok:
        return {
            "ok": True, "uploaded": False, "skipped": True,
            "message": _reason,
            "results": [{"name": "CPA 过滤", "ok": True, "msg": _reason}],
        }
