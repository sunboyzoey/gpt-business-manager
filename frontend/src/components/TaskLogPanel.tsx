...
export function TaskLogPanel({ taskId, onDone, showControls = true, doneText = '注册完成', maxLines }: TaskLogPanelProps) {
  ...
  const limitLines = (items: string[]) =>
    lineLimit > 0 ? items.slice(-lineLimit) : items
  ...
  useEffect(() => {
    ...
    const initSnapshot = async (): Promise<boolean> => {
      try {
        const snapshot = await apiFetch(`/tasks/${taskId}${lineLimit > 0 ? `?tail=${lineLimit}` : ''}`) as {
          logs?: string[]
          logs_total?: number
          logs_offset?: number
        ...
        setLines(limitLines(snapshotLines))
        nextSinceRef.current = Number(
          snapshot.logs_total ?? (
            Number(snapshot.logs_offset || 0) + snapshotLines.length
          )
        )
    ...
    const connectStreamOnce = async (): Promise<boolean> => {
      try {
        ...
        const tailParam = lineLimit > 0 ? `&tail=${lineLimit}` : ''
        const response = await fetch(`${API_BASE}/tasks/${taskId}/logs/stream?since=${since}${tailParam}`, {
          headers,
          ...
          for (const part of parts) {
            ...
            try {
              ...
              if (payload.line) {
                ...
                setLines((previous) => limitLines([...previous, payload.line!]))
              }
              if (payload.done) {
