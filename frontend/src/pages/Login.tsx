...
type Step = 'checking' | 'setup' | 'password' | '2fa'
...
function LoginContent() {
  ...
  useEffect(() => {
    ...
    fetch('/api/auth/status', { signal: controller.signal }).then(async response => {
      ...
      setStep(state.has_password ? 'password' : 'setup')
    }).catch(reason => {
      if (!controller.signal.aborted) message.error(`读取登录状态失败：${String(reason)}`)
    })
  ...
  if (step === 'checking') {
    return <div style={wrapStyle}><Spin size="large" /></div>
  }
      ...
      <div style={wrapStyle}>
        <Card
          ...
          title={
            <div style={{ textAlign: 'center', padding: '8px 0' }}>
              ...
              <div style={{ fontSize: 18, fontWeight: 700 }}>初始化管理员</div>
        ...
        >
          <Form layout="vertical" onFinish={handleSetup} requiredMark={false}>
            <Form.Item name="password" label="管理员密码" rules={[
              { required: true, message: '请输入管理员密码' },
              { min: 8, message: '密码至少需要 8 位' },
            ]}>
              <Input.Password prefix={<LockOutlined />} placeholder="至少 8 位" size="large" autoComplete="new-password" />
            </Form.Item>
      ...
      <Card
        ...
        title={
          <div style={{ textAlign: 'center', padding: '8px 0', background: 'transparent' }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 700 }}>Gmail · BUSINESS 管理</div>
            <Typography.Text type="secondary" style={{ fontSize: 13, fontWeight: 400 }}>
              请输入密码登录
            </Typography.Text>
          </div>
