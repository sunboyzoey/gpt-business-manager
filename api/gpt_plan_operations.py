"""GPT 套餐账号的 PRO/OAuth/设备兼容操作。

该模块仍保留部分 ``gpt_pro`` 命名，仅为 BUSINESS/统一设备调用方的
内部 API 兼容。账号和操作租约的唯一运行时真源分别是
``GptPlanAccountModel`` 和 ``GptPlanAccountOperationLeaseModel``；不再读写
``gpt_pro_accounts`` 或旧租约表。``/gpt-pro`` router 已从主应用退役；
保留此模块只是为了让 BUSINESS/统一设备复用稳定的操作原语。
"""
...
from sqlalchemy import and_, or_, delete as sa_delete, update as sa_update
...
def _mail_provider_of(acc) -> str:
    ...
    return account_mail_provider(acc)


def _gmail_binding_extra(acc) -> dict:
    from services.gmail_plan_support import gmail_binding_extra
    return gmail_binding_extra(acc) if _mail_provider_of(acc) == "gmail" else {}
...
class GptProAccountUpdateRequest(BaseModel):
    ...
    mail_provider: Optional[str] = None    # outlook / icloud / gmail
...
def list_gpt_pro_accounts(
    ...
    mail_provider: Optional[str] = Query(None, description="邮箱类型: outlook / icloud / gmail"),
    ...
    with Session(engine) as session:
        ...
        if mail_provider is not None:
            ...
            if mp in {"icloud", "qqmail", "outlook", "gmail"}:
                query = query.where(mail_provider_expression(GptProAccountModel) == ("icloud" if mp == "qqmail" else mp))
...
def _reclaim_gpt_pro_account_operation_before_remote_write(
    account_id: int,
    operation: str,
    previous_token: str,
    *,
    ...
    """Replace a vanished caller-owned lease only before any device write.

    Missing-credential reconciliation first refreshes the stored OAuth token and
    only afterwards persists the CPA/Sub2API remote-write intent.  A process or
    rolling-recovery race can remove that caller-owned lease in the small gap
    after an invalid RT is reported.  It is safe to reclaim exactly there, but
    never after an upload intent (or its compensation audit) exists.

    The intent check and replacement lease insert share one database transaction,
    so a competing account operation cannot slip between them.  This helper is
    intentionally internal and accepts only the two delivery providers.
    """
    prior = str(previous_token or "").strip()
    normalized_provider = str(provider or "").strip().lower()
    normalized_operation = str(operation or "").strip()
    if not prior or normalized_provider not in {"cpa", "sub2api"}:
        raise HTTPException(409, "设备凭证修复租约不能安全重建")
    now = _utcnow()
    replacement = _operation_uuid.uuid4().hex
    try:
        with Session(engine) as session:
            is_sqlite = session.get_bind().dialect.name == "sqlite"
            if is_sqlite:
            ...
            if current and (_aware_utc(current.expires_at) or now) > now:
                if (
                    str(current.token or "") == prior
                    and str(current.operation or "") == normalized_operation
                ):
                    session.rollback()
                    return prior
                raise HTTPException(409, {
                    "code": "gpt_pro_account_busy",
                    "message": "子号已由其他账号操作接管",
                    "operation": str(current.operation or "account_operation"),
                    "expires_at": _iso_utc(current.expires_at),
                })
            if current:
                session.delete(current)
            ...
            if normalized_provider == "cpa":
                remote_write_evidence = bool(
                    extra.get("cpa_sync_pending")
                    or extra.get("cpa_sync_compensation_pending")
                    or str(extra.get("cpa_sync_pending_token") or "") == prior
                    or str(extra.get("cpa_sync_compensation_token") or "") == prior
                )
            else:
                remote_write_evidence = bool(
                    extra.get("sub2api_sync_pending")
                    or str(
                        extra.get("sub2api_sync_pending_operation_token") or ""
                    ) == prior
                )
            if remote_write_evidence:
                raise HTTPException(
                    409,
                    "设备远端写入已开始，拒绝重建旧凭证修复租约",
                )

            session.add(GptProAccountOperationLeaseModel(
                account_id=int(account_id),
                operation=normalized_operation or "account_operation",
                token=replacement,
...
def update_gpt_pro_account(account_id: int, body: GptProAccountUpdateRequest):
    with Session(engine) as session:
        ...
        if body.mail_provider is not None:
            ...
            if p not in {"icloud", "qqmail", "outlook", "gmail"}:
                raise HTTPException(400, "邮箱类型仅支持 outlook、icloud 或 gmail")
            p = "icloud" if p == "qqmail" else p
            acc.mail_provider = p
            # Preserve the legacy JSON marker while compatibility endpoints
            # still exist, but the typed plan column is authoritative.
            try:
...
def _reacquire_parent_codex_rt(acc: GptProAccountModel, log=None) -> bool:
    ...
    from services.gmail_plan_support import gmail_receive_ready, gmail_binding_extra
    if _provider != "icloud" and not (acc.password or "").strip() and not (_provider == "gmail" and gmail_receive_ready(acc)):
    ...
    try:
        ...
        try:
            ...
            _log(f"[补 RT] 已挂载 {'iCloud/QQ' if _prov == 'icloud' else 'Gmail' if _prov == 'gmail' else 'outlook'} 收件箱 OTP 适配器(从账号邮箱读验证码)")
        except Exception as exc:
            if _provider == "gmail":
                _log("[补 RT] Gmail 收件适配器不可用，请检查母号收件授权")
                return False
...
_REFERRAL_CF_DOMAIN_KEY = "gpt_plan_referral_cf_domain"               # 老:单值(兼容)
_REFERRAL_CF_DOMAINS_KEY = "gpt_plan_referral_cf_domains_json"        # 新:清单 JSON
...
def _referral_complete_core(pro_account_id, email, proxy,
    ...
    try:
        ...
        reply_wait = max(0, min(300, int(_csr.get("gpt_plan_app_reply_wait_seconds", 25) or 25)))
    except Exception:
        reply_wait = 25
    if reply_wait:
    ...
    try:
        reward_timeout = max(30, min(1800, int(_cs2.get("gpt_plan_reward_wait_seconds", 300) or 300)))
    except Exception:
        reward_timeout = 300
    with Session(engine) as s:
...
_CPA_SYNC_TARGETS_KEY = "gpt_plan_cpa_sync_targets"
_CPA_SYNC_TARGETS_REVISION_KEY = "gpt_plan_cpa_sync_targets_revision"
    ...
    *,
    ...
    with _CPA_CONFIG_WRITE_LOCK:
        with Session(engine) as session:
            ...
            # Hosting workers pin this exact target. The shared settings page
            # must honor the same fence as the device-management endpoint.
            from services.device_hosting_store import has_active_workers, forget_device
            for hosting_id, before_target in old_by_id.items():
                after_target = new_by_id.get(hosting_id)
                if before_target != after_target and has_active_workers(f"cpa:{hosting_id}", session=session):
                    raise HTTPException(409, "CPA 设备仍有执行中的子号托管步骤，请暂停并等待该步骤结束后修改")
                if after_target is None:
                    forget_device(f"cpa:{hosting_id}", session=session)
            ...
            # A BUSINESS mother binding is durable directory metadata even
            # though TEAM-child automation is retired.  Protect every bound
            # policy, not just old auto-rotation rows, so a target cannot be
            # deleted/repointed behind the binding CRUD screen.
            try:
                for policy in session.exec(
                    ...
                    provider, provider_id = resolve_business_delivery_binding(
                        policy
                    )
                    if provider == "cpa" and provider_id is not None:
                        protected_ids.add(provider_id)
            except OperationalError as exc:
                # Narrow legacy compatibility only.  Any other read failure
                # must fail closed or a bound target could be deleted behind
                # the local binding directory.
                if "no such table" not in str(exc).lower():
                    raise HTTPException(
                        503,
                        "BUSINESS 母号设备绑定暂不可读取，请稍后重试",
                    ) from exc
...
def _resolve_cpa_endpoint(ex: dict) -> tuple:
    ...
    if domain == _CPA_CONFIG_DOMAIN_AUTO_POOL:
        try:
            ...
            device_id = 0
        if device_id:
            from services.gpt_plan_cpa_manager import _auto_pool_endpoint_by_device_id
...
_SUB2API_AUTO_REMOVE_CONFIG_KEY = "gpt_plan_sub2api_auto_remove"
...
def _sub2api_link_of(acc: GptProAccountModel) -> Optional[dict]:
    ...
    extra = _extra_of(acc)
    # A successful device inventory has authoritative absence semantics.  Keep
    # the historical remote id in extra_json for audit/fencing, but never
    # advertise it as an active linkage until a later exact upload verification
    # clears this marker.
    if extra.get("sub2api_remote_missing") is True:
        return None
    try:
...
def _sync_account_to_cpa_claimed(
    ...
    *,
    ...
    allow_credential_reauth: bool = True,
    ...
    ``gpt_plan_cpa_sync_targets``，调用方不能传任意 URL / API key。
    """
    if type(allow_destructive_recovery) is not bool:
        raise ValueError("allow_destructive_recovery must be a boolean")
    if not allow_destructive_recovery:
        _require_cpa_delivery_guard(delivery_guard, "只新增同步准备")
        current = _require_cpa_account_scope(account_id, expected_business_parent_id=expected_business_parent_id)
        current_extra = _extra_of(current)
        if any(current_extra.get(key) for key in (
            "cpa_sync_pending", "cpa_sync_compensation_pending", "cpa_migration_pending",
            "cpa_disable_pending", "cpa_cleanup_pending", "cpa_disabled", "cpa_monitor_suppressed",
        )):
            raise HTTPException(409, "CPA 已有待核对或停用状态，只新增托管不会恢复旧任务")
        if current_extra.get("cpa_synced") and (
            int(current_extra.get("cpa_synced_to_id") or 0) != int(target_id)
            or str(current_extra.get("cpa_config_domain") or "") not in {"", _CPA_CONFIG_DOMAIN_MANUAL}
        ):
            raise HTTPException(409, "CPA 已关联其他设备，只新增托管不执行迁移或停用")
    recovery = ({"active": False} if not allow_destructive_recovery else _promote_stale_cpa_sync_pending(
        account_id,
        operation_token=operation_token,
    ))
    ...
    compensation = {"ok": True} if not allow_destructive_recovery else _attempt_cpa_sync_compensation(account_id)
    ...
    if not (acc.codex_refresh_token or "").strip():
        if not allow_credential_reauth:
    ...
    credential_kwargs: dict[str, Any] = {"_require_persist": True}
    if not allow_credential_reauth:
        credential_kwargs["_allow_reauth"] = False
    _fresh_access_token, chatgpt_account_id = (
        _gpt_pro_resolve_codex_credentials(acc, **credential_kwargs)
    )
    ...
    if switching_target:
        if not allow_destructive_recovery:
            raise HTTPException(409, "CPA 设备归属已变化，只新增托管不执行迁移或停用")
        if not old_api_url:
    ...
    try:
        ...
        if not allow_destructive_recovery:
            _mark_cpa_sync_intent_error(account_id, "托管状态在上传后变化，保留待核对，不停用远端凭据", operation_token=operation_token)
            raise HTTPException(409, "托管上传结果待核对，未停用远端凭据") from guard_exc
        auth_name = f"{current_email}.json"
    ...
    if not bool(probe.get("ok")):
        ...
        if not allow_destructive_recovery:
            _mark_cpa_sync_intent_error(account_id, "上传后凭据健康未确认，保留待核对，不停用远端凭据", operation_token=operation_token)
            raise HTTPException(502, "CPA 上传后健康核验失败，未标记完成；保留待核对，不停用远端凭据")
        persisted = _persist_cpa_sync_compensation_intent(
            account_id,
            operation_token=operation_token,
            target_id=int(tgt["id"]),
    ...
    try:
        ...
        if not allow_destructive_recovery:
            _mark_cpa_sync_intent_error(account_id, "上传后账号归属变化，保留待核对，不停用远端凭据", operation_token=operation_token)
            raise
        if _gpt_pro_account_operation_token_is_current(account_id, operation_token):
            _patch_gpt_pro_extra(account_id, {
    ...
    *,
    ...
    try:
        return _sync_account_to_cpa_claimed(
            ...
            allow_credential_reauth=allow_credential_reauth,
            **({"allow_destructive_recovery": False} if not allow_destructive_recovery else {}),
        )
    finally:
        if owned_token:
...
def get_account_cpa_usage(
    ...
    *,
    ...
    from services.cpa_manager import list_auth_files
    from services.gpt_plan_cpa_manager import _fetch_account_usage
    try:
        files = list_auth_files(api_url=api_url, api_key=api_key)
    except Exception as e:
    ...
    *,
    ...
    try:
        ...
        if replacement_credentials is not None:
            ...
            if allow_credential_reauth:
                fresh_access_token, _chatgpt_account_id = (
                    _gpt_pro_resolve_codex_credentials(account)
                )
            else:
                fresh_access_token, _chatgpt_account_id = (
...
def _auto_refund_enabled() -> bool:
    from core.config_store import config_store as _cs
    return str(_cs.get("gpt_plan_cpa_auto_refund_enabled", "1")).strip().lower() not in ("0", "false", "no", "off", "")
    ...
    *,
    ...
    try:
        ...
        if not endpoints or unresolved_pending_target:
            ...
            return False
        try:
            from services.gpt_plan_cpa_manager import _delete_account_files_by_email
...
# 用一个全局可配置的 CF 邀请域名(`gpt_plan_referral_cf_domain` config key),
...
def _phase_invite_and_reward(ctx: Dict[str, Any], log) -> Dict[str, Any]:
    ...
    try:
        reward_timeout = max(30, min(1800, int(_cs_reward.get("gpt_plan_reward_wait_seconds", 300) or 300)))
    except Exception:
        reward_timeout = 300
...
def _fetch_gmail_recent(acc: GptProAccountModel, limit: int) -> List[Dict[str, Any]]:
    from platforms.chatgpt.gpt_pro_login import build_mailbox_for_account
    from services.gmail_plan_support import gmail_binding_extra
    mailbox, mb_account = build_mailbox_for_account({"email": acc.email,
        "mail_provider": "gmail", **gmail_binding_extra(acc)})
    return mailbox.list_recent(mb_account, limit=limit)


def _fetch_recent_for_account(acc: GptProAccountModel, limit: int) -> List[Dict[str, Any]]:
    """按 provider 分发取件；Gmail/icloud 均固定匹配完整子号地址。"""
    if _mail_provider_of(acc) == "gmail":
        return _fetch_gmail_recent(acc, limit)
...
def _apply_payment_account_by_last4(session, acc: GptProAccountModel, last4: str) -> Optional[str]:
    ...
    if not l4:
        return None
    from core.db import GptPlanCardModel as _Card, GptPlanPaymentAccountModel as _PA
...
def fetch_account_mail(account_id: int, body: Optional[GptProFetchMailRequest] = None):
    ...
    with Session(engine) as session:
        ...
        if _mail_provider_of(acc) == "gmail":
            try:
                messages = _fetch_gmail_recent(acc, limit)
            except Exception:
                raise HTTPException(400, "Gmail 取件失败，请检查母号可用状态与收件授权") from None
            acc.last_used = _utcnow()
            session.add(acc)
            session.commit()
            return {"email": acc.email, "mail_access_type": "gmail", "method": "gmail",
...
def _run_drission_login(
    ...
    *,
    ...
    # 可用 config gpt_plan_auto_delete_dead_on_login=1 且账号非 is_pro 时才允许删除(默认关闭)。
    # Only the shared browser login's explicit terminal verdict is authority.
    # A timeout/exception mentioning an error-code URL is not deactivation.
    ...
    if _login_dead:
        ...
        try:
            ...
            auto_del = str(_cs.get("gpt_plan_auto_delete_dead_on_login", "0") or "0").strip().lower() in ("1", "true", "yes")
        except Exception:
            auto_del = False
        deleted = False
        with Session(engine) as session:
...
def _upgrade_pro_phase2_thread_impl(task_id: str, account_id: int, page,
    ...
    from core.db import GptPlanCardModel as CardModel
    from api.gpt_plans import _fetch_plan_bank_cards
        ...
        with Session(engine) as session:
            ...
            if picked_card_id:
                ...
                if card:
                    ...
                    try:
                        from core.config_store import config_store as _cs
                        max_uses = int(str(_cs.get("gpt_plan_card_max_uses", "") or 6) or 6)
                    except Exception:
                        max_uses = 6
                    ...
                    if acc and getattr(card, "payment_account_id", 0):
                        try:
                            from core.db import GptPlanPaymentAccountModel as _PA
...
def _upgrade_pro_account_claimed(
    ...
    # 可用配置 gpt_plan_ph_go_first=0 关掉回到直接 PRO。
    try:
        from core.config_store import config_store as _cs
        _ph_go_first_cfg = str(_cs.get("gpt_plan_ph_go_first", "") or "1").strip() != "0"
        _go_plan_name = str(_cs.get("gpt_plan_go_plan_name", "") or "chatgptgoplan").strip() or "chatgptgoplan"
        _go_to_pro_wait = int(str(_cs.get("gpt_plan_go_to_pro_wait_seconds", "") or 60) or 60)
    except Exception:
...
def get_cpa_config():
    from services import gpt_plan_cpa_manager as m
    return m.get_config()
...
def _assert_managed_business_oauth_owner(
    ...
    *,
    ...
    Its authority is the exact active pool membership.  Direct OAuth accepts a
    pending invite too; the post-OAuth workspace refresh determines whether it
    has become a member instead of inferring membership from a saved RT.
    """
    if lock_for_update and session.get_bind().dialect.name == "sqlite":
        session.connection().exec_driver_sql("BEGIN IMMEDIATE")
    account_query = select(GptProAccountModel).where(
...
def oauth_account_for_managed_business_child(
    ...
    *,
    ...
    """Start OAuth for one active managed BUSINESS pool child.

    The default composite mode confirms/joins the target workspace first.  The
    explicit direct mode is reserved for a caller whose invitation flow already
    completed the child login; it skips that second login and accepts either an
    active remote invite or a confirmed remote member as the ownership fence.
    """
    if type(allow_phone_verification) is not bool:
        raise HTTPException(400, "allow_phone_verification 必须是布尔值")
    if expected_invited_at is not None and (not isinstance(expected_invited_at, str) or not expected_invited_at.strip()):
        raise HTTPException(400, "expected_invited_at 必须是非空时间戳")
    body = body or GptProActionRequest()
    with Session(engine) as session:
        try:
    ...
    with _MANAGED_BUSINESS_OAUTH_LOCK:
        with _AUTO_INVITE_LOCK:
            for existing_task_id, task in _AUTO_INVITE_TASKS.items():
                if (
                    ...
                    if task.get("managed_business_allow_phone_verification", True) is not allow_phone_verification:
                        raise HTTPException(409, "该子号已有不同手机号验证许可的 RT 任务正在执行")
                    if task.get("managed_business_expected_invited_at") != expected_invited_at:
                        raise HTTPException(409, "该子号已有不同邀请周期恢复约束的 RT 任务正在执行")
        ...
        with _AUTO_INVITE_LOCK:
            ...
            task.update({
                ...
                "managed_business_allow_phone_verification": allow_phone_verification,
                "managed_business_expected_invited_at": expected_invited_at,
            })
        worker_options = {
        ...
        th = threading.Thread(
            ...
            **({"kwargs": worker_options} if worker_options else {}),
            daemon=True,
        )
        try:
            th.start()
...
def _run_managed_business_child_oauth_task(
    ...
    try:
        ...
        if direct_oauth:
            _log("步骤 1/1：邀请阶段已完成登录，直接开始 Codex OAuth")
            _run_oauth_task(
                task_id,
                int(account_id),
                body,
                ...
                managed_remote_member_required=False,
        **({"allow_phone_verification": False} if not allow_phone_verification else {}),
        **({"expected_invited_at": expected_invited_at} if expected_invited_at is not None else {}),
                **({"allow_existing_credentials": True} if allow_existing_credentials else {}),
    )
        ...
        _run_oauth_task(
            ...
            expected_managed_business_parent_id=int(parent_id),
            expected_managed_operation_token=required_operation_token,
            managed_remote_member_required=True,
            **({"allow_phone_verification": False} if not allow_phone_verification else {}),
            **({"expected_invited_at": expected_invited_at} if expected_invited_at is not None else {}),
            **({"allow_existing_credentials": True} if allow_existing_credentials else {}),
        )
        ...
        _log(
            f"✗ 直达 OAuth 任务异常: {error}"
            if direct_oauth
            else f"✗ 登录/进入工作区任务异常: {error}"
        )
        _invite_task_finish(
            task_id,
            error=error,
            result={
                "stage": (
                    "oauth_start_failed" if direct_oauth else "workspace_join_failed"
                ),
                "oauth_failure": normalize_oauth_failure(getattr(exc, "oauth_failure", None))
                    or OAuthAttemptFailure("oauth_unknown", charge_possible=True).oauth_failure,
            },
        )
...
def _oauth_failure_is_account_deactivated(value: Any) -> bool:
    """Recognize the stable Codex/OpenAI account-deactivated terminal reason."""
    failure = normalize_oauth_failure(getattr(value, "oauth_failure", None))
    if failure is not None:
        return failure["code"] == "account_deactivated"
    text = str(value or "").strip().lower().replace("-", "_").replace(" ", "_")
    return bool(
        "account_deactivated" in text
        or "access_deactivated" in text
        or "account_has_been_deactivated" in text
        or "account_has_been_deleted_or_deactivated" in text
    )


def _mark_oauth_account_deactivated(
    account_id: int,
    task_id: str,
    *,
    expected_business_parent_id: Optional[int] = None,
    ...
    """Persist a dead-account marker only while the OAuth binding still matches.

    The allocation worker needs a durable terminal fact because the in-memory
    task snapshot disappears on process restart.  This helper never deletes an
    account or changes its BUSINESS membership; the fenced allocation cleanup
    owns the later exact remove/revoke and history transition.
    """
    now = _utcnow()
    with Session(engine) as session:
        if session.get_bind().dialect.name == "sqlite":
        ...
        if not acc:
            return False
        parent_id = int(getattr(acc, "business_parent_id", None) or 0)
        if (
            expected_business_parent_id is not None
            and parent_id != int(expected_business_parent_id)
        ):
            session.rollback()
            return False
        if (
            expected_managed_business_parent_id is not None
            and parent_id != int(expected_managed_business_parent_id)
        ):
            session.rollback()
            return False
        required_managed_token = str(
            expected_managed_operation_token or ""
        ).strip()
        if required_managed_token:
            lease_query = select(GptProAccountOperationLeaseModel).where(
                GptProAccountOperationLeaseModel.account_id == int(account_id)
            )
            if session.get_bind().dialect.name != "sqlite":
                lease_query = lease_query.with_for_update()
            lease = session.exec(lease_query).first()
            if not (
                lease
                and str(lease.token or "") == required_managed_token
                and (_aware_utc(lease.expires_at) or now) > now
            ):
                session.rollback()
                return False
        extra = _extra_of(acc)
        if expected_business_parent_id is not None and (
            str(extra.get("business_rotation_oauth_operation_id") or "")
            != str(expected_operation_id or "")
            or str(extra.get("business_rotation_oauth_task_id") or "")
            != str(task_id or "")
        ):
            session.rollback()
            return False
        acc.dangerous = True
        if not acc.dangerous_detected_at:
            acc.dangerous_detected_at = now
        extra["codex_oauth_account_deactivated"] = True
        extra["codex_oauth_account_deactivated_at"] = now.isoformat()
        extra["codex_oauth_account_deactivated_task_id"] = str(task_id or "")
        if expected_business_parent_id is not None:
            extra["business_rotation_oauth_status"] = "account_deactivated"
            extra["business_rotation_oauth_finished_at"] = now.isoformat()
            extra.pop("business_rotation_oauth_lease_expires_at", None)
        acc.extra_json = json.dumps(extra, ensure_ascii=False)
        acc.updated_at = now
        session.add(acc)
...
def _enqueue_saved_child_oauth_reconciliation(account_id: int, parent_id: int, log_fn) -> None:
    """Notify NV of stored credentials, never launch another OAuth or upload.

    An active canonical account lease may defer this notification. The normal
    NV dispatcher repeats the same local evidence check after it is released.
    Failure here must not turn successfully stored credentials into RT failure.
    """
    try:
        from services.nv_automation_store import recover_saved_oauth_jobs

        result = recover_saved_oauth_jobs(
            child_id=int(account_id), source_account_id=int(parent_id),
        )
        if result.get("queued", 0):
            log_fn("自动售号已进入已保存 RT 核对队列；不重新授权、不申请短信")
    except Exception:
        log_fn("RT 已保存；自动售号状态衔接暂未完成，调度器将再次检查已有凭证")


def _refresh_managed_child_workspace_after_oauth(
    account_id: int,
    parent_id: int,
    log_fn,
) -> dict:
    """Reconcile the real workspace once after durable OAuth credential storage.

    Single and batch RT callers share this worker.  Refresh the mother's normal
    capability here, not in a particular frontend polling loop, so normalized
    membership and the persisted workspace snapshot move together.  OAuth
    success alone is never evidence of membership; a failed/cached read leaves
    the saved RT intact and reports a separate, visible warning.
    """
    result = {
    ...
    try:
        ...
        with Session(engine) as session:
            child = _assert_managed_business_oauth_owner(
                session, account_id, parent_id, require_remote_member=False,
            )
            email = str(child.email or "").strip().lower()
        log_fn("RT 已保存，正在刷新目标 BUSINESS 工作区成员状态")
        workspace = workspace_members(int(parent_id))
        if not isinstance(workspace, dict) or not workspace.get("fresh") or workspace.get("cached"):
            raise RuntimeError("本次远端刷新未成功，仍为旧成员快照")
        if any(
            isinstance(row, dict)
            and str(row.get("email") or "").strip().lower() == email
            for row in workspace.get("membership_conflicts", [])
        ):
            raise RuntimeError("该子号成员记录写回冲突")
        member = next((
            row for row in workspace.get("members", [])
            if isinstance(row, dict) and not row.get("deactivated")
            and str(row.get("email") or "").strip().lower() == email
        ), None)
        pending = next((
            row for row in workspace.get("invites", [])
            if isinstance(row, dict)
            and str(row.get("email") or "").strip().lower() == email
        ), None)
        # A removal/rebind while reading must not be reported as joined merely
        # because the response contains an identity from the old workspace.
        with Session(engine) as session:
            child = _assert_managed_business_oauth_owner(
                session, account_id, parent_id, require_remote_member=False,
            )
            if str(child.email or "").strip().lower() != email:
                raise RuntimeError("子号身份已变化，请重新刷新当前归属")
            membership = session.exec(
                select(GptBusinessChildMembershipModel)
                .where(GptBusinessChildMembershipModel.business_account_id == int(parent_id))
                .where(GptBusinessChildMembershipModel.pro_account_id == int(account_id))
                .where(GptBusinessChildMembershipModel.ended_at.is_(None))
                .where(func.lower(GptBusinessChildMembershipModel.email) == email)
                .where(func.lower(GptBusinessChildMembershipModel.source) == "pool")
            ).first()
            if member and (
                not membership or not str(member.get("user_id") or "").strip()
                or membership.remote_user_id != str(member["user_id"])
                or bool(membership.remote_invite_id)
            ):
                raise RuntimeError("远端已确认加入，但本地成员记录尚未同步")
            if not member and pending and (
                not membership or not str(pending.get("invite_id") or "").strip()
                or membership.remote_invite_id != str(pending["invite_id"])
                or bool(membership.remote_user_id)
            ):
                raise RuntimeError("远端仍为待接受邀请，但本地邀请记录尚未同步")
        status = "member" if member else "pending" if pending else "local"
        result.update(workspace_refreshed=True, workspace_status=status)
        if status == "member":
            log_fn("工作区状态已更新：已加入；成员记录与工作区快照已保存")
        else:
            result["workspace_refresh_warning"] = (
                "RT 已保存，但远端仍显示待接受邀请，请确认工作区加入状态"
                if status == "pending" else
                "RT 已保存，但远端未找到该子号的成员或邀请记录，请检查工作区归属"
            )
    except Exception as exc:
        # Unknown network/SQL exceptions can include credentials or bound SQL
        # parameters. Only emit exact local diagnostics, never their raw repr.
        safe_reasons = {
            "本次远端刷新未成功，仍为旧成员快照",
            "该子号成员记录写回冲突",
            "子号身份已变化，请重新刷新当前归属",
            "远端已确认加入，但本地成员记录尚未同步",
            "远端仍为待接受邀请，但本地邀请记录尚未同步",
            "OAuth 子号不存在",
            "OAuth 子号健康状态或 BUSINESS 归属已变化",
            "OAuth 子号缺少邮箱",
            "OAuth 子号已不再是该母号的活跃池选子号",
            "OAuth 子号缺少可确认的 BUSINESS 邀请或成员绑定",
        }
        reason = str(exc) if str(exc) in safe_reasons else "远端读取或本地保存发生异常"
        if isinstance(exc, HTTPException):
            reason = (
                "母号正在执行其他操作，或刷新结果已失效"
                if exc.status_code == 409 else f"工作区刷新返回 HTTP {int(exc.status_code)}"
            )
        result["workspace_refresh_warning"] = (
            f"RT 已保存，但工作区状态刷新未完成：{reason}；可点击刷新成员/席位重试"
        )
    if result["workspace_refresh_warning"]:
        log_fn(f"⚠ {result['workspace_refresh_warning']}")
    return result


def _managed_business_oauth_writeback_snapshot(
    session: Session, account: GptProAccountModel, parent_id: int,
) -> dict[str, Any]:
    """Keep the original credential and membership identity only in memory."""
    memberships = session.exec(select(GptBusinessChildMembershipModel).where(
        GptBusinessChildMembershipModel.business_account_id == int(parent_id),
        GptBusinessChildMembershipModel.pro_account_id == int(account.id),
        GptBusinessChildMembershipModel.ended_at.is_(None),
        func.lower(GptBusinessChildMembershipModel.email) == str(account.email or "").strip().lower(),
        func.lower(GptBusinessChildMembershipModel.source) == "pool",
    )).all()
    if len(memberships) != 1:
        raise RuntimeError("OAuth 子号邀请周期不唯一，未启动授权")
    membership = memberships[0]
    if membership.invited_at is None and membership.created_at is None:
        raise RuntimeError("OAuth 子号缺少邀请周期，未启动授权")
    return {
        "email": account.email,
        "credentials": {key: getattr(account, key) for key in (
            "codex_access_token", "codex_refresh_token", "codex_id_token",
            "codex_session_token", "codex_rt_acquired_at",
        )},
...
def _persist_managed_business_oauth_cas(
    session: Session, account_id: int, parent_id: int, operation_token: str,
    observed: dict[str, Any], values: dict[str, Any],
) -> None:
    """The write itself fences credentials, the invitation cycle and the lease."""
    member = GptBusinessChildMembershipModel
    remote_user = func.coalesce(member.remote_user_id, "")
    remote_invite = func.coalesce(member.remote_invite_id, "")
    remote_identity = (
        remote_user == observed["remote_user_id"] if observed["remote_user_id"] else or_(
            and_(remote_user == "", remote_invite == observed["remote_invite_id"]),
            and_(remote_user != "", remote_invite == ""),
        )
    )
    cycle = [member.invited_at == observed["invited_at"]]
    if observed["invited_at"] is None:
        cycle.append(member.created_at == observed["created_at"])
    membership_current = select(member.id).where(
        member.id == observed["membership_id"],
        member.business_account_id == int(parent_id),
        member.pro_account_id == int(account_id),
        member.ended_at.is_(None),
        func.lower(member.email) == str(observed["email"] or "").strip().lower(),
        func.lower(member.source) == "pool", *cycle, remote_identity,
    ).exists()
    lease_current = select(GptProAccountOperationLeaseModel.account_id).where(
        GptProAccountOperationLeaseModel.account_id == int(account_id),
        GptProAccountOperationLeaseModel.token == operation_token,
        GptProAccountOperationLeaseModel.expires_at > _utcnow(),
    ).exists()
    try:
        written = session.exec(sa_update(GptProAccountModel).where(
            GptProAccountModel.id == int(account_id),
            GptProAccountModel.email == observed["email"],
            GptProAccountModel.business_parent_id == int(parent_id),
            GptProAccountModel.enabled == True,  # noqa: E712
            GptProAccountModel.dangerous == False,  # noqa: E712
            GptProAccountModel.policy_warning == False,  # noqa: E712
            func.coalesce(GptProAccountModel.refund_status, "") == "",
            *(getattr(GptProAccountModel, key) == value for key, value in observed["credentials"].items()),
            membership_current, lease_current,
        ).values(**values).execution_options(synchronize_session=False))
    except Exception:
        # SQL exceptions can include bound credentials; never relay them.
        raise RuntimeError("OAuth 凭证写回未确认，已停止覆盖") from None
    if int(getattr(written, "rowcount", 0) or 0) != 1:
        raise RuntimeError("OAuth 期间凭证、邀请周期或操作租约已变化，未覆盖已有凭证")
...
def _run_oauth_task(
    ...
    *,
    ...
    expected_managed_business_parent_id: Optional[int] = None,
    expected_managed_operation_token: str = "",
    managed_remote_member_required: bool = True,
    allow_phone_verification: bool = True,
    expected_invited_at: str | None = None,
    allow_existing_credentials: bool = False,
) -> None:
    ...
    try:
        ...
        managed_writeback_snapshot = None
        expected_cycle = None
        if expected_invited_at is not None:
            try:
                expected_cycle = datetime.fromisoformat(expected_invited_at.replace("Z", "+00:00"))
                if expected_cycle.tzinfo is None:
                    raise ValueError("timezone missing")
            except (TypeError, ValueError, AttributeError):
                raise RuntimeError("OAuth 恢复缺少准确邀请周期，未启动授权") from None
            if not required_managed_operation_token or expected_managed_business_parent_id is None:
                raise RuntimeError("OAuth 恢复缺少子号操作租约，未启动授权")
        if (
        ...
        with Session(engine) as session:
            acc = (
                _assert_managed_business_oauth_owner(
                    ...
                    require_remote_member=bool(managed_remote_member_required),
                    lock_for_update=True,
                )
            ...
            if required_managed_operation_token and expected_managed_business_parent_id is not None:
                managed_writeback_snapshot = _managed_business_oauth_writeback_snapshot(
                    session, acc, int(expected_managed_business_parent_id),
                )
                if expected_cycle is not None:
                    observed = managed_writeback_snapshot
                    cycle = _aware_utc(observed["invited_at"] or observed["created_at"])
                    if cycle != expected_cycle:
                        raise RuntimeError("OAuth 恢复邀请周期已变化，未启动授权")
                    if (not allow_existing_credentials
                            and any(str(value or "").strip() for value in observed["credentials"].values())):
                        raise RuntimeError("OAuth 恢复前已出现凭证或获取时间记录，未启动授权或覆盖")
        ...
        if not allow_phone_verification:
            _log("本次 OAuth 禁止手机号验证；若页面要求手机号，将停止且不申请短信号码")
        elif extra_config.get("smsbower_api_key"):
            _log(
                "已注入 smsbower 配置 "
        ...
        # Prepare before browser navigation so an extra verification email is
        # not mistaken for old mail.  Mail is optional for password/TOTP-only
        # login; the browser boundary fails closed if a challenge needs it.
        email_adapter = None
        try:
            mailbox, mb_account = build_mailbox_for_account(snapshot, proxy=proxy)
        ...
        try:
            ...
            _log(f"✗ OAuth 失败: {exc}")
            account_deactivated = _oauth_failure_is_account_deactivated(exc)
            oauth_failure = normalize_oauth_failure(getattr(exc, "oauth_failure", None))
            if oauth_failure is None:
                oauth_failure = OAuthAttemptFailure(
                    "account_deactivated" if account_deactivated else "oauth_unknown", charge_possible=True,
                ).oauth_failure
            if account_deactivated:
                _mark_oauth_account_deactivated(
                    account_id,
                    task_id,
                    expected_business_parent_id=expected_business_parent_id,
                    expected_operation_id=expected_operation_id,
                    expected_managed_business_parent_id=(
                        expected_managed_business_parent_id
                    ),
                    expected_managed_operation_token=(
                        required_managed_operation_token
                    ),
                )
            if expected_business_parent_id is not None:
                _record_business_rotation_oauth_terminal(
                    ...
                    expected_operation_id,
                    task_id,
                    "account_deactivated" if account_deactivated else "failed",
                )
            _invite_task_finish(
                task_id,
                error=("account_deactivated" if account_deactivated else str(exc)),
                result={
                    "stage": (
                        "account_deactivated"
                        if account_deactivated else "oauth_failed"
                    ),
                    "code": (
                        "account_deactivated" if account_deactivated else ""
                    ),
                    "dead": account_deactivated,
        ...
        if not rt:
            ...
            _invite_task_finish(
                ...
                result={"stage": "no_refresh_token", "oauth_failure": OAuthAttemptFailure(
                    "partial_credentials", callback_received=True, exchange_started=True,
        ...
        with Session(engine) as session:
            ...
            if acc:
                ...
                elif managed_writeback_snapshot is not None:
                    _persist_managed_business_oauth_cas(
                        session, account_id, int(expected_managed_business_parent_id),
                        required_managed_operation_token, managed_writeback_snapshot,
                        {"codex_access_token": at, "codex_refresh_token": rt,
                         "codex_id_token": idt, "codex_session_token": sess,
                         "codex_rt_acquired_at": completed_at, "updated_at": completed_at},
                    )
                else:
        ...
        workspace_result = {}
        if expected_managed_business_parent_id is not None:
            workspace_result = _refresh_managed_child_workspace_after_oauth(
                account_id, int(expected_managed_business_parent_id), _log,
            )
            if (workspace_result.get("workspace_refreshed") is True
                    and workspace_result.get("workspace_status") == "member"):
                _enqueue_saved_child_oauth_reconciliation(
                    account_id, int(expected_managed_business_parent_id), _log,
                )
        _invite_task_finish(task_id, result={
            "stage": "done",
...
def _download_pro_oauth_file(
    ...
    *,
    ...
    assigned_proxy_url = ""
    try:
        from services.gpt_plan_cpa_proxy_pool import assign_proxy
...
def _serialize(acc: GptProAccountModel) -> Dict[str, Any]:
    ...
    return {
        ...
        "has_oauth": (True if _mail_provider_of(acc) == "icloud" else
                      gmail_receive_ready(acc)
                      if _mail_provider_of(acc) == "gmail" else bool(acc.client_id and acc.refresh_token)),
