...
class RegisterTaskRecord:
    ...
    def _speed_snapshot(self) -> dict[str, Any]:
        now = time.time() if self.status in {"pending", "running"} else self.updated_at
        elapsed = max(float(now - self.created_at), 0.0)
        failed = len(self.errors)
        completed = int(self.success or 0) + int(self.skipped or 0) + failed

        success_per_second = (self.success / elapsed) if elapsed > 0 else 0.0
        attempts_per_second = (completed / elapsed) if elapsed > 0 else 0.0
        return {
            "elapsed_seconds": elapsed,
            "completed": completed,
            "failed": failed,
            "success_per_second": success_per_second,
            "seconds_per_success": (elapsed / self.success) if self.success > 0 else 0.0,
            "attempts_per_second": attempts_per_second,
            "seconds_per_attempt": (elapsed / completed) if completed > 0 else 0.0,
        }
            ...
            for record in self._records.values():
                ...
                if meta:
                    ...
                    for key, value in meta.items():
                        if str(record.meta.get(key, "")) != str(value):
                            matched = False
                            break
                    if not matched:
                        continue
