"""旧 GPT PRO 界面专属的升级浏览器策略。

GPT 套餐管理使用 ``core.gpt_plan_upgrade_browser_config`` 和独立代理表，
不会读写本模块的配置。这里的配置只在旧 GPT PRO ``upgrade-pro`` 入口按
请求快照解析。
"""
...
from .db import RoxyProxyModel, engine as default_engine


UPGRADE_BROWSER_BACKEND_KEY = "gpt_upgrade_browser_backend"
UPGRADE_ROXY_PROXY_ID_KEY = "gpt_upgrade_roxy_proxy_id"
...
def load_upgrade_browser_config() -> dict[str, Any]:
    """读取旧 GPT PRO 配置；历史数据库没有配置时回退到本地浏览器。"""
...
def save_upgrade_browser_config(
    *,
    ...
    """校验并原子保存旧 GPT PRO 策略；切回 local 时清空已选代理。"""
...
def resolve_upgrade_browser_selection(
    ...
    *,
    ...
    """把请求显式值与 GPT PRO 配置合并为本次升级的不可变快照。

    兼容规则：

    * 显式 ``browser_backend`` 永远优先于 GPT PRO 配置；
    * 显式选择 ``roxybrowser`` 但省略代理，保留旧语义（Roxy 无指定代理）；
    * 省略浏览器字段时才继承 GPT PRO 配置及其代理；
