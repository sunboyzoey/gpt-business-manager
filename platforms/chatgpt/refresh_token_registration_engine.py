...
class RegistrationResult:
    ...
    source: str = "register"
    # True only when the remote signup password request was confirmed.  A
    # generated password by itself is not proof that the account accepted it.
    password_set_proven: bool = False
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "source": self.source,
            "password_set_proven": self.password_set_proven,
        }
...
class RefreshTokenRegistrationEngine:
    ...
    def __init__(
        ...
        self._allow_phone_verification = str(
            self.extra_config.get("chatgpt_rt_allow_phone_verification") or "0"
    ...
    def run(self) -> RegistrationResult:
        ...
        last_result = RegistrationResult(success=False, logs=self.logs)

        for attempt in range(self.max_retries):
            if attempt:
                self._log(f"整流程重试 {attempt + 1}/{self.max_retries} ...", "warning")
                time.sleep(1)

            # 未显式指定邮箱时，每轮必须从邮箱服务获取新地址；
            # 不能把上一轮写入 self.email 的地址误判为预设邮箱。
            self.email = fixed_email or None
            last_result = self._run_single_attempt()
            if last_result.success:
                return last_result
            if last_result.error_code == "user_already_exists":
                return last_result
            if not self._should_retry(last_result.error_message):
                return last_result

        return last_result

    def _run_single_attempt(self) -> RegistrationResult:
        ...
        try:
            ...
            self._log(f"密码: 已生成/已提供（长度 {len(self.password)}，内容不写入日志）")
            ...
            resume_login = self.extra_config.get("gmail_registration_resume") is True
            if resume_login:
                # A crashed request may already have created the identity. Resume
                # through login; don't submit a second sign-up speculatively.
                self._log("2. 恢复 Gmail 子号：从邮箱登录继续上次未完成的步骤")
                registered, registration_message = False, "user_already_exists: gmail_resume"
            else:
            ...
            result.password_set_proven = (
                getattr(register_client, "last_password_set_proven", False) is True
            )

            if (not resume_login and self.extra_config.get("mail_provider") == "gmail"
                    and getattr(register_client, "last_registration_error_code", "") == "user_already_exists"):
                result.error_code = "user_already_exists"
                result.error_message = "Gmail 注册明确返回 user_already_exists，停止当前母号的新号生产"
                return result
            ...
            stage_callback = self.extra_config.get("_registration_stage_callback")
            if callable(stage_callback):
                stage_callback("oauth")
            ...
            if use_continued_session:
                ...
                tokens = oauth_client.login_and_get_tokens(
                    ...
                    allow_phone_verification=self._allow_phone_verification,
            ...
            if not tokens:
                if (self.extra_config.get("mail_provider") == "gmail"
                        and getattr(oauth_client, "last_registration_error_code", "") == "user_already_exists"):
                    result.error_code = "user_already_exists"
                    result.error_message = "Gmail 注册明确返回 user_already_exists，停止当前母号的新号生产"
                    return result
                last_error = oauth_client.last_error or "OAuth 登录状态机失败"
                if resume_login and any(marker in str(last_error).lower() for marker in (
                    "account_not_found", "user_not_found", "no account found", "account does not exist",
                )):
                    self._log("已确认此邮箱尚无 GPT 账号，继续原子号的注册步骤")
                    self.extra_config["gmail_registration_resume"] = False
