...
class TokenRefreshManager:
    ...
    def _response_error_detail(response) -> str:
        """Return a compact OAuth error detail for callers that need classification."""
        try:
            data = response.json()
        except Exception:
        ...
        if not isinstance(data, dict):
            ...
            return f" - {text[:300]}" if text else ""
        parts: list[str] = []
        for key in ("error", "code", "error_description", "message"):
            value = data.get(key)
        ...
        if isinstance(nested, dict):
            for key in ("code", "type", "message"):
                ...
                if value:
                    parts.append(f"error.{key}={value}")
        if parts:
    ...
    def refresh_by_oauth_token(
        ...
        try:
            ...
            # curl_cffi 走代理偶发 TLS 崩(curl 35 OPENSSL_internal:invalid library),
            # 重试 + 每次重建 session 规避坏掉的 SSL 句柄;HTTP 4xx(如 refresh_token_reused)不重试
            response = None
            last_exc = None
            import time as _t
            for _attempt in range(1, 4):
                try:
                    ...
                    last_exc = net_exc
                    logger.warning(f"OAuth refresh 第 {_attempt}/3 次网络异常: {net_exc}")
                    _t.sleep(2)
                    try:
                        session = self._create_session()
                    except Exception:
                        pass
            if response is None:
                result.error_message = f"OAuth token 刷新异常(网络重试 3 次仍失败): {last_exc}"
                logger.error(result.error_message)
                return result
            ...
            if response.status_code != 200:
                ...
                result.error_message = (
                    f"OAuth token 刷新失败: HTTP {response.status_code}{detail}"
                )
