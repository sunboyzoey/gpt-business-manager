...
def _build_compat_id_token(
    *,
    ...
    organization_id = str(auth_info.get("organization_id") or f"org-{hashlib.sha1((account_id or email_from_token or user_id).encode('utf-8'), usedforsecurity=False).hexdigest()[:24]}")
    project_id = str(auth_info.get("project_id") or f"proj_{hashlib.sha1((organization_id + ':' + (account_id or user_id)).encode('utf-8'), usedforsecurity=False).hexdigest()[:24]}")
    ...
    compat_payload = {
        ...
        "jti": f"compat-{hashlib.sha1(access_token.encode('utf-8'), usedforsecurity=False).hexdigest()[:32]}",
...
def generate_token_json(account) -> dict:
    """
    生成 CPA 格式的 Token JSON (最小化版本)。

    仅包含 3 个字段: type / access_token / refresh_token。
    其他字段 (email/id_token/account_id/expired/last_refresh/session_token) 全部不输出,
    避免 session_token (ST) 泄漏到磁盘文件。
    """
    result = {
        "type": "codex",
    ...
    priority = getattr(account, "cpa_priority", 0)
    if priority:
...
def upload_to_cpa(
    ...
    upload_filename = str(filename or "").strip()
    if not upload_filename:
        token_email = str(token_data.get("email") or "").strip()
        upload_filename = f"{token_email}.json" if token_email else "chatgpt_oauth.json"
...
def upload_auth_file_to_cpa(
    ...
    """上传 CPA auth-file JSON 文件内容（不走代理）。
    api_url / api_key 为空时自动从 ConfigStore 读取。"""
    if not api_url:
    ...
    filename = str(filename or "auth.json").strip() or "auth.json"
