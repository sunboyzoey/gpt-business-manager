...
def generate_oauth_url(
    *,
    ...
    # 注:旧版同时带 prompt=login / id_token_add_organizations=true / codex_cli_simplified_flow=true,
    # 其中 codex_cli_simplified_flow=true 会触发 OpenAI 对 CLI 注册的额外手机验证。
    # 浏览器注册不带这几个参数,这里模拟浏览器以避免 add_phone 风控。
    # 通过环境变量 OAUTH_CLI_SIMPLIFIED_FLOW=1 可以恢复旧行为。
    import os as _os
    params = {
        "client_id": client_id,
        "response_type": "code",
    ...
    if _os.environ.get("OAUTH_CLI_SIMPLIFIED_FLOW", "").strip().lower() in ("1", "true", "yes"):
        params.update({
            "prompt": "login",
