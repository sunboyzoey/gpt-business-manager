...
class AccessTokenOnlyRegistrationEngine:
    ...
    def _log(self, message: str, level: str = "info"):
        message = str(message)
        if self._active_password:
            message = message.replace(self._active_password, "[REDACTED]")
    ...
    def run(self) -> RegistrationResult:
        ...
        registration_password = self.password or generate_random_password()
        self._active_password = registration_password
        try:
            last_error = ""
                ...
                try:
                    ...
                    if not success:
                        if (self.extra_config.get("mail_provider") == "gmail"
                                and getattr(chatgpt_client, "last_registration_error_code", "") == "user_already_exists"):
                    ...
                    if session_ok:
                        ...
                        result.password_set_proven = (
                            getattr(chatgpt_client, "last_password_set_proven", False)
                            is True
                        )
