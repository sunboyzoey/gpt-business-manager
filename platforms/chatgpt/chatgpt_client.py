...
class ChatGPTClient:
    ...
    def __init__(self, proxy=None, verbose=True, browser_mode="protocol"):
        ...
        self.last_stage = ""
        # Only becomes true after the remote password-registration request has
        # succeeded.  Callers must not infer this from a generated password or
        # from the overall login/token flow succeeding.
    ...
    def verify_email_otp(self, otp_code, return_state=False):
        ...
        Returns:
            tuple: (success, message)
        """
        self._enter_stage("otp", "verify email otp（内容不写入日志）")
        self._log("正在验证 OTP（内容不写入日志）")
    ...
    def register_complete_flow(
        ...
        # A client can be reused by retrying callers.  Proof belongs to this
        # individual registration attempt, so never carry it across runs.
        self.last_password_set_proven = False
        self.last_registration_error_code = ""
